...
func (b *BrokerStore) GetConsumer(ctx context.Context, id string, op messagebroker.ConsumerClientOptions) (messagebroker.Consumer, error) {
	...
	startTime := time.Now()
	defer brokerStoreOperationTimeTaken.WithLabelValues(env, "GetConsumer").Observe(float64(time.Now().Sub(startTime).Milliseconds() / 1e3))
...
func (b *BrokerStore) RemoveConsumer(_ context.Context, id string, op messagebroker.ConsumerClientOptions) bool {
	...
	startTime := time.Now()
	defer brokerStoreOperationTimeTaken.WithLabelValues(env, "RemoveConsumer").Observe(float64(time.Now().Sub(startTime).Milliseconds() / 1e3))
...
func (b *BrokerStore) GetProducer(ctx context.Context, op messagebroker.ProducerClientOptions) (messagebroker.Producer, error) {
	...
	startTime := time.Now()
	defer brokerStoreOperationTimeTaken.WithLabelValues(env, "GetProducer").Observe(float64(time.Now().Sub(startTime).Milliseconds() / 1e3))
...
func (b *BrokerStore) GetAdmin(ctx context.Context, options messagebroker.AdminClientOptions) (messagebroker.Admin, error) {
	...
	startTime := time.Now()
	defer brokerStoreOperationTimeTaken.WithLabelValues(env, "GetAdmin").Observe(float64(time.Now().Sub(startTime).Milliseconds() / 1e3))
