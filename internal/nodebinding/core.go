...
func (c *Core) CreateNodeBinding(ctx context.Context, m *Model) error {
	...
	startTime := time.Now()
	defer nodeBindingOperationTimeTaken.WithLabelValues(env, "CreateNodeBinding").Observe(float64(time.Since(startTime).Nanoseconds() / 1e9))
...
func (c *Core) Exists(ctx context.Context, key string) (bool, error) {
	...
	startTime := time.Now()
	defer nodeBindingOperationTimeTaken.WithLabelValues(env, "Exists").Observe(float64(time.Since(startTime).Nanoseconds() / 1e9))
...
func (c *Core) ListKeys(ctx context.Context, prefix string) ([]string, error) {
	...
	startTime := time.Now()
	defer nodeBindingOperationTimeTaken.WithLabelValues(env, "ListKeys").Observe(float64(time.Since(startTime).Nanoseconds() / 1e9))
...
func (c *Core) List(ctx context.Context, prefix string) ([]*Model, error) {
	...
	startTime := time.Now()
	defer nodeBindingOperationTimeTaken.WithLabelValues(env, "List").Observe(float64(time.Since(startTime).Nanoseconds() / 1e9))
...
func (c *Core) DeleteNodeBinding(ctx context.Context, m *Model) error {
	...
	startTime := time.Now()
	defer nodeBindingOperationTimeTaken.WithLabelValues(env, "DeleteNodeBinding").Observe(float64(time.Since(startTime).Nanoseconds() / 1e9))
