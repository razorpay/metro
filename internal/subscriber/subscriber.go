...
func (s *Subscriber) Run(ctx context.Context) {
	for {
		...
		case req := <-s.requestChan:
			...
			// putting inside anonymous func block so that `defer` can be wrapped in
			// cannot `defer` without this as the code block is running within a loop
			func() {
