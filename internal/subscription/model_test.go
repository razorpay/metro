...
func TestModel_Prefix(t *testing.T) {
	...
	assert.Equal(t, "registry/projects/test-project/subscriptions/", dTopic.Prefix())
}

func TestModel_Key(t *testing.T) {
	dTopic := getDummySubscriptionModel()
	assert.Equal(t, "registry/projects/"+dTopic.ExtractedSubscriptionProjectID+"/subscriptions/"+dTopic.ExtractedSubscriptionName, dTopic.Key())
}
